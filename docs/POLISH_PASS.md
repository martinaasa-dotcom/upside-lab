...
The boxes below were not ticked as the work landed, so this file spent a
day describing a third of a finished pass as outstanding. That is the exact
failure the pass spent its time removing from every other document here, so
it is worth saying how it was fixed rather than quietly flipping them.

Every open and in-progress line was checked against the tree rather than
against memory: the second-pass security findings by the artefact each
names (`portfolioCreatorId` on the owners route, `escapeLike` and
`likeCanBeExact` on unsubscribe, the zod schemas behind chat, holdings
import, conviction and forecast, `purge-session.ts`, `safeAvatarUrl`,
`chargeUnresolvedBudget`, `TICKER_RE`, the invite hash), the reader-copy
and design work by the tests written for it (`landing-claims`,
`reader-copy`, `provenance`, `glossary`, `Explain`, `welcome-tour`,
`account-copy`, `fitment`, `heading-scale`), the tier gates by reading
`TIER_HIDDEN_META_TABS` and finding all three tiers empty, and the
`COOKIES.md` honesty fix by reading the paragraph itself. All of it had
landed in #180.

Two things genuinely moved that day rather than being re-labelled: the
...
- [x] Second pass at full depth: each dimension re-audited by several independent finders, every finding refuted by independent skeptics before it is acted on, a completeness critic asking what was missed.
- [x] Every fix validated by the repo's own checks before it lands; every new behaviour gets a test that would have failed before the change.
- [x] Merge to `main`, then a full audit pass against the live build.
...
- [x] Chat `ccContext` and `messages` reach the system prompt unshaped.
- [x] Holdings import skips the money and share ceilings the single-holding route enforces.
...
- [x] What is worth doing instead, and is not done: `'unsafe-inline'` is why `docs/COOKIES.md` can say the CSP is the defence for a cookie that is deliberately not HttpOnly. That argument is weaker than the file makes it sound, and the honest compensating controls are the ones already in place (no `'strict-dynamic'`, a named-host `connect-src` and `img-src`, the same-origin gate on every mutation). Worth saying so in `COOKIES.md` rather than leaving the CSP sounding stronger than it is.
- [x] Unsubscribe matches email with `ilike` (LIKE wildcards).
- [x] `/auth/link` acts on GET (mail scanners confirm an address).
- [x] Raw error strings in three responses.
- [x] `/api/trends` on the per-instance limiter only. Now the durable limiter, per reader.
- [x] Circle invite tokens stored in plaintext beside their hash.
- [x] Unshaped `conviction` and `forecast.rows` bodies; a malformed row is a 500.
- [x] `/api/book/nav-history` serves anonymous callers. Requires a session now.
- [x] Second-pass finders: authorization and IDOR; input validation and injection; auth flows, cookies and crypto; abuse and rate limits; supply chain (`npm audit`, lockfile).

## 2. Performance, caching, page speed

- [x] Two remote auth round trips per API call (proxy plus route handler).
...
- [x] `/api/upside-portfolio` recomputed per viewer.
- [x] Client fetch dedupe for quotes, fear and greed, portfolios, experience tier, market events.
- [x] Redundant FX-only quote poll.
...
- [x] Second-pass finders: server latency, client waterfalls and polling, bundle and render cost, CDN and browser caching, measured before and after.

## 3. Copy, voice, accuracy

- [x] Eight sentences that are instructions to buy, sell or hold, on the covered-call surfaces, the Pulse schema and the Sunday letter.
- [x] Jargon without a plain gloss (ROI, P&L, equity, strike, yield, allocation, benchmark, presidential cycle, leverage multiple).
...
- [x] One name for the Sunday letter, one name for a circle, companies rather than "names".
- [x] Walls: the walkthrough's 62-word opening sentence, the terms' 200-word payment paragraph.
- [x] Empty states and error messages that do not say what to do next.
- [x] Every historical or market claim checked for accuracy.
- [x] Second-pass finders per screen, plus a claims checker with web verification.

## 4. Margus

- [x] Persona: "risk reference point", the borrowing ceiling that reads as a rule.
...
- [x] Response speed: streaming, first-token time, chunk warm.
- [x] Openers, empty portfolio behaviour, what he can and cannot write.
- [x] Second-pass: an adversarial reader tries to make Margus give advice, use slang, or invent a number.
...
- [x] Holdings entry: screenshot, CSV, paste and typing all reachable from the first screen that asks.
- [x] Empty Home reads as a wall.
...
- [x] Ideas to critique, and every one of the six came through the judge panel and was built: the plain-word glossary on tap (`glossary.ts` behind `Explain` and `TermTip`), the one-minute read triggered by your own day (`typical-move.ts` and `market-or-you.ts`, on Home and Pulse), the paper portfolio to practise with (the classroom portfolio, which was already the shape of this), the weekly reflection (the Sunday letter, which is the same idea with a stamp on it), streaks that reward looking rather than trading (`visit-streak.ts`, on the Dashboard and Account). This line named `streaks.ts` until 2026-09-11 and was wrong twice over: that module was imported by nothing, had no test of its own, and counted consecutive up days of a *price*, which is a run to chase rather than a reason to look. It has been deleted. The claim a few lines above, that every module here is imported by a component that is itself mounted, was checked against the tree rather than against this list and held for all of them but that one, and "what would you have to believe" per holding (`believe.ts`, on the forecast and in the ticker drawer).
...
- [x] Forecast described as a room in three places.
- [x] README's room list.

## 10. Documentation and rules

- [x] Read every rule in `AGENTS.md`, `DESIGN_TOKENS.md` and `docs/` against the goal. Loosen the ones that cost more than they protect, correct the ones that are wrong, and record why.

## 11. Bloat and removal

- [x] Unused exports, unreachable routes, unused dependencies, features nobody can reach.

## 12. Tests and CI

- [x] A test per fix above. Invariants suite still green. CI runtime.

## 13. Merge and the live pass

- [x] Merge to `main`.
- [x] Walk the production build room by room, phone and laptop, and record what it found in `docs/MVP_AUDIT_LIVE_PASS.md`.
...
- [x] A co-owner can remove the portfolio's creator and lock them out (the RLS fix closed only the direct path).
- [x] Linked-address confirmation can bind a stranger's future account to the requester (never-signed-up address, spent on GET).
- [x] A tab or newline in `next` survives `safeInternalPath` and becomes an open redirect after sign-in.
- [x] Session cookies set without `Secure`.
- [x] Rate-limit buckets keyed on an unverified cookie value.
- [x] Proxy matcher skips every path containing a dot, so dotted slugs get no CSP and no forged-request gate.
- [x] Junk symbols in one anonymous quote request cost thousands of provider calls before the budget is charged.
- [x] A ticker with a bracket, stored by import, crashes Pulse for every co-owner.
- [x] Call %, targets and sort order accept any finite number.
- [x] Account purge leaves a deleted person's holdings inside older saved copies.
- [x] An expired session skips the client purge, so the next account on the browser inherits notes and the sync queue.
- [x] Google fallback binds accounts by email string, not the provider's subject.
- [x] Any avatar URL works as a tracking pixel on every circle member.
- [x] Add-address endpoint tells any signed-in user whether an address has an account.
- [x] One-time tokens are spent without checking the row count.
- [x] The sign-in link page never names the address it opens and replaces an existing session silently.
...
- [x] Circle invite tokens stored in plaintext (in flight from the first pass).
- [x] Unsubscribe `ilike`, `/auth/link` on GET, raw error strings, holdings import bounds, unshaped chat and forecast bodies (in flight from the first pass).
...
- [x] Opening a circle is four routes times three auth round trips plus four serial waves each.
- [x] Margus waits for two rate-limit RPCs and a sixteen-ticker calendar before the first token.
- [x] Account mounts four routes that each read the same profile row.
- [x] Client fetch sharing, the FX-only poll, the 1 Hz timers, the Margus chunk loader (waiting on the Dashboard package).
...
- [x] Home: hero that leads with the value, market card that answers "me or the market", one briefing card, no duplicate notices, honest chart caption, watchlist suggestions that are not three coins, "less than 1%" everywhere, status strip in words.
- [x] Portfolio room: fractional shares, Gain instead of ROI, phone card with a lead figure, tap-to-open drawer, drawer that matches the row, totals up top, covered calls hidden below 100 shares, placeholder forecast drawn as a placeholder, modal copy.
- [x] Pulse: the day's story first, the market's move beside yours, company names on cards, a real measured range, fallback cards that say so, headlines with source and age, no forced model call from Home.
...
- [x] Circle: percent-only by default, a duel that resolves, reasons shared side by side, an empty circle that leads with the invite, what changed since you looked, a League that fits on a phone, one award per person, "circle" not "community".
...
- [x] `TIER_HIDDEN_META_TABS` hides Lab from a novice, which is the analysis room withheld from exactly the reader the product is for. The answer is Lab with each tab saying what to notice, not Lab hidden, so this waits on the Lab content package.
- [x] `TIER_HIDDEN_LAB_TABS` hides Risk until advanced. A shock test is one of the most teaching things in the app.
...
- [x] The three fixture rows are still in `portfell_error_log` and are displacing real errors from the admin console's recent window. One button at `/admin`, and clearing resets the digest's comparison window by design. Left for Martin rather than reaching into production data.
