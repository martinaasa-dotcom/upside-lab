...
| Spending layers | `--layer-essentials`, `--layer-regular`, `--layer-discretionary`, `--layer-luxuries` | The retirement room's "what a bad year costs you" stack, and nothing else. Teal, blue, gold and pink, light (L 0.74-0.84) and saturated (C 0.13-0.16) so each layer is told apart at a glance and all four take the dark `--primary-foreground` ink. They replaced a one-hue warm ramp that read as four shades of khaki. **None is `--gain` or `--loss`**, which is the fault the ramp had been fixing: groceries are not drawn in the colour of a profit. Not a general categorical set; `--cat-*` is that. |
