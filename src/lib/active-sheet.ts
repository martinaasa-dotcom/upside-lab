...
/*
  `OPEN_TAB_KEY` and its stash/take pair lived here, holding the tab a dock
  cell had asked for across a client navigation that dropped the `?tab=`
  carrying it. Rooms are paths now and a path is not dropped, so the
  question the stash answered cannot be asked. Nothing reads the key; it is
