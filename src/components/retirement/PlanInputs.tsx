...
type TopicProps = {
  ...
  patch: Patch;
  /** Unticks this topic's chip. */
  onClose: () => void;
};

export function HomeTopic({ inputs, patch, onClose }: TopicProps) {
  ...
  return (
    <Panel className="relative">
      <TopicClose topic="home" onClose={onClose} />
      <PanelHeader
        className="pr-10"
...
export function ChildrenTopic({ inputs, patch, onClose }: TopicProps) {
  const region = regionById(inputs.regionId);
  const code = currencyCodeFor(region.currency);
  return (
    <Panel className="relative">
      <TopicClose topic="children" onClose={onClose} />
      <PanelHeader
        className="pr-10"
...
export function CarTopic({ inputs, patch, onClose }: TopicProps) {
  const region = regionById(inputs.regionId);
  const code = currencyCodeFor(region.currency);
  const standardCosts = costAnchorsForStandard(inputs.standard);
  return (
    <Panel className="relative">
      <TopicClose topic="car" onClose={onClose} />
      <PanelHeader
        className="pr-10"
...
export function SavingsTopic({ inputs, patch, onClose }: TopicProps) {
  const region = regionById(inputs.regionId);
  const code = currencyCodeFor(region.currency);
  const costsId = useId();
  return (
    <Panel className="relative">
      <TopicClose topic="savings" onClose={onClose} />
      <PanelHeader
        className="pr-10"
...
export function IncomeTopic({ inputs, patch, onClose }: TopicProps) {
  const region = regionById(inputs.regionId);
  const code = currencyCodeFor(region.currency);
  return (
    <Panel className="relative">
      <TopicClose topic="income" onClose={onClose} />
      <PanelHeader
        className="pr-10"
